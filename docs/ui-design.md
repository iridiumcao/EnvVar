...
| 文件 | 导出 | 将全部变量导出为 JSON |
| | 导入 | 从 JSON 文件导入变量 |
| | 打开元数据目录 | 在资源管理器中打开存储目录 |
| | 退出 | 关闭应用 |
| 语言 | English / 简体中文 / 繁體中文 | 切换界面语言并持久化 |
| 帮助 | 关于 | 显示应用信息 |
...
| Value | 多行文本框（等宽字体） | 变量值，默认紧凑显示，滚动查看超长内容 |

字段标签列采用自动宽度（SharedSizeGroup），确保中英文模式下标签与输入框间距一致且紧凑。
...
- **历史**：弹出当前变量的历史版本列表（最多 5 条），选择后将 Value、Alias、Description 加载到编辑器（不自动保存）；仅编辑已有变量时可用
